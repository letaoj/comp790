...
public class AnEditorInteractor implements ListObserver {
	...
	public AnEditorInteractor(SimpleList list, Communicator communicator) {
		this.list = list;
	...
	protected void processAddTopic(int index, String input) {
		ListEditInput.newCase(OperationName.ADD, list.size(), input,
	...
	protected void processRemoveTopic(int index, String input) {
		ListEditInput.newCase(OperationName.DELETE, list.size(), input,
				ApplicationTags.EDITOR, this);
		selfDisplay = false;
		for (int i = 0; i < input.length(); i++) {
