...
public class UI extends JFrame {
  ...
  public UI(ListObserver<String> historyInteractor, ListObserver<Character> topicInteractor) {
    ...
    this.addWindowListener(new WindowAdapter() {
      public void windowClosing(WindowEvent e) {
        ((AnIMInteractor) historyInteractor).leave();
        ((AnEditorInteractor) topicInteractor).leave();
        UI.this.dispose();
      }
    });
    initUI();
      ...
      public void removeUpdate(DocumentEvent e) {
        try {
          ...
          for (int i = 0; i < e.getLength(); ++i) {
            topicInteractor.processRemoveTopic(offset, e.getDocument().getText(offset, 1));
          }
        } catch (BadLocationException e1) {
          e1.printStackTrace();
        }
